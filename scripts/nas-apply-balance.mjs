// Paket 1 deployment: NAS only, verified migration ledger, backup, atomic guards.
import {readFileSync,writeFileSync,mkdirSync} from 'node:fs';
import {spawnSync} from 'node:child_process';
import {homedir} from 'node:os';
import {createHash} from 'node:crypto';
const name='20260908121725_fair_balance_and_permanent_xp.sql';
...
if(ledger.some(m=>m.name===name)){console.log('Balance migration already applied with matching hash.');process.exit(0);}
if(ledger.length!==9)throw Error('Expected nine verified baseline migrations.');
console.log('NAS baseline: nine matching migration hashes.');
...
const backup='/volume2/Datenbanken/AuraQuest/backups/balance-before-'+stamp+'.dump';
...
do $$ begin
 ...
 if (select count(*) from public.player_xp_events)<>(select count(*) from public.check_ins) then raise exception 'XP backfill confirmation count mismatch'; end if;
end $$;
`;
const result=sql(`begin;set local lock_timeout='15s';set local statement_timeout='120s';select public.lock_game_state();
do $$ begin if (select count(*) from aura_admin.applied_migrations)<>9 then raise exception 'Baseline changed'; end if;end $$;
${guards}\n${migration}\n${verify}
